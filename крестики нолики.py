...
print("  Приветсвуем вас  ", "      в игре  ", " крестики-нолики!", " формат ввода: xy", "x - номер строки  ",
      "y - номер столбца ",  sep="\n")
...
def print_game_map(gm):
  print("  0  1  2 ")
  for row_index, row in enumerate(gm):
    print(row_index, end=" ")
    for cell in row:
...
def random_player(gm, player):          # Бот
...
def input_player(gm, player):
    while True:
        ...
        if len(player_input) != 2:
            print("Введено неверное количество символов. Формат: xy.")
            continue
...
game_players = [None, input_player, input_player]
